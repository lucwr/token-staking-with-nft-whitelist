...
async function DeployBATICU() {
  // deploting the Bored ape tokens Initial coin offering
  const BATICUContract = await ethers.getContractFactory("buyBAT");
  const BAT_ICU = await BATICUContract.deploy(BATContract);
